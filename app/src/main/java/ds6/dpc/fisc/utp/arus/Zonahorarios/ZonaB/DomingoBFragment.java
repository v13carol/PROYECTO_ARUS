...
public class DomingoBFragment extends Fragment {
    ...
    {    View  rootView = inflater.inflate(R.layout.fragment_domingo_b, container, false);

        cnn = new DbHelper(getActivity(), Esquema.Zona.DATABASE_NAME,null, Esquema.Zona.DATABASE_VERSION);
        list_zona = (ListView) rootView.findViewById(R.id.list_domingob);
        consultar_lista();
    ...
    private void consultar_lista() {
        ...
        cursor = db.rawQuery("SELECT "+ Esquema.Zona.DOMINGO+" FROM "+ Esquema.Zona.TABLE_NAME2,null);
