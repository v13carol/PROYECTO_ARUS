...
public class MartesBFragment extends Fragment {
    ...
    {    View  rootView = inflater.inflate(R.layout.fragment_martes_b, container, false);

        cnn = new DbHelper(getActivity(), Esquema.Zona.DATABASE_NAME,null, Esquema.Zona.DATABASE_VERSION);

        list_zona = (ListView) rootView.findViewById(R.id.list_martesb);
        consultar_lista();
    ...
    private void consultar_lista() {
        ...
        cursor = db.rawQuery("SELECT "+ Esquema.Zona.MARTES+" FROM "+ Esquema.Zona.TABLE_NAME2,null);
