...
public class LunesBFragment extends Fragment {
    ...
    {    View  rootView = inflater.inflate(R.layout.fragment_lunes_b, container, false);

        cnn = new DbHelper(getActivity(), Esquema.Zona.DATABASE_NAME,null, Esquema.Zona.DATABASE_VERSION);
        list_zona = (ListView) rootView.findViewById(R.id.list_lunesb);
        consultar_lista();
    ...
    private void consultar_lista() {
        ...
        cursor = db.rawQuery("SELECT "+ Esquema.Zona.LUNES+" FROM "+ Esquema.Zona.TABLE_NAME2,null);
